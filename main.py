...
from mlir import passmanager, ir
...
def run_mlir_passes(mlir_text: str, passes: list[str]) -> str:
  """Run a list of MLIR passes on the MLIR text and return transformed text."""
  with ir.Context() as ctx:
    mlir_module = ir.Module.parse(mlir_text, context=ctx)
...
def main():
  ...
  xdsl_module = Parser(ctx, sample_stencil).parse_module()

  # Convert IR from xDSL stencil to LLVM dialects 
  lowering_pass = ConvertStencilToLLMLIRPass()


  lowering_pass.apply(ctx, xdsl_module)
  ...
  lowered_ir = ""
  if device_type == "cpu":
    lowered_ir = run_mlir_passes(mlir_text=mlir_text, passes=cpu_passes)
  elif device_type == "gpu":
    lowered_ir = run_mlir_passes(mlir_text=mlir_text, passes=gpu_passes)
  else:
    print(f"Unrecognised device type {device_type}")
    exit(1)

  # Convert the 0R in the module to a string object
